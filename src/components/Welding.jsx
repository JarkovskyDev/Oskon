    ...
    <div name='welding' className='w-full min-h-screen'>
       <div className='max-w-[1400px] m-auto px-4 grid lg:grid-cols-2 gap-4 sm:pt-0 sm:pb-0'>
          ...
          <div className='flex flex-col h-full justify-center'>
            <h3 name='welding-res' className='text-5xl font-bold max-sm:mt-2 sm:mt-2 max-sm:order-1 sm:order-1'>
              <span className='border-t-2 border-[#ea5b0c]'>S</span>VAŘOVÁN<span className='border-b-2 border-[#ea5b0c]'>Í</span>
            </h3>
            <p className='text-xl text-justify pb-6 mt-4 max-sm:order-2 sm:order-2 sm:pb-0'>Oskon s.r.o je odborníkem na oblast svařování a kovových výrobků spojených s touto technikou. Naše firma se zaměřuje na výrobu vysokojakostních svařovaných komponent pro různé odvětví průmyslu.  Také se zaměřujeme na svařování  konstrukcí ve stavebnictví, a to svařování  ocelových nosníků, nosných sloupků, rámových konstrukcí a dalších svařovaných prvků, které zajišťují stabilitu a bezpečnost budov. S důrazem na preciznost a bezpečnost jsme vaším spolehlivým partnerem pro kovové výrobky, které vám pomáhají dosáhnout excelentních svařovacích výsledků.</p>
          </div>
        </div>     
    </div>
  )
